...
in
{
  ...
  systemd.services.llama-cpp-server = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${llamaPackage}/bin/llama-server --port 8001 --host 0.0.0.0 --models-preset /opt/llm/llama-cpp.ini --offline -ngl 99 --threads 8 --log-verbosity 2 --load-mode dio --flash-attn on --ctx-checkpoints 0 --fit off --metrics";
      Restart = "on-failure";
      RestartSec = "5s";
    };
  };
