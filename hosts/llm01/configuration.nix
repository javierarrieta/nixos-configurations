...
in
{
  ...
  systemd.services.llama-cpp-server = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${llamaPackage}/bin/llama-server --port 8001 --host 0.0.0.0 --models-preset /opt/llm/llama-cpp.ini --flash-attn on --no-mmap --offline -ngl 99 --threads 16 --log-verbosity 2";
      Restart = "on-failure";
      RestartSec = "5s";
    };
  };
