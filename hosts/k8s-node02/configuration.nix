...
in
{
  ...
  sops = {
    ...
    secrets."k8s-node02/network_env" = {
      mode = "0400";
      owner = "root";
    };
    secrets."ssh_keys/k8s-node02_host_private" = {
      mode = "0600";
      owner = "root";
      path = "/etc/ssh/ssh_host_ed25519_key";
    };
    secrets."ssh_keys/k8s-node02_host_public" = {
      mode = "0644";
      owner = "root";
  ...
  systemd.services."network-addresses-enp3s0".serviceConfig.EnvironmentFile =
    lib.mkForce
      config.sops.secrets."k8s-node02/network_env".path;
