...
**Architecture:** New repo holds the derivation (`image.nix`), a minimal flake exposing `packages.x86_64-linux.coder-workspace`, a GitHub Actions workflow that builds the image and pushes `YYYYMMDD-<short-sha>` + `latest` to `ghcr.io/javierarrieta/coder-workspaces-nix`, and a fresh `IMAGE_TAGS.md`. Infra repo deletes `pkgs/coder-workspace/`, the old workflow, `IMAGE_TAGS.md`, and the flake entry. Consumer repoint (`coder-templates`) is a flagged follow-up, not part of this repo's change.
...
- Canonical publish target: GHCR only — `ghcr.io/javierarrieta/coder-workspaces-nix`. No LAN registry pushes.
- New repo local path: `/home/coder/.cache/coder-workspaces` (subagents run as a different uid and cannot write under `/home/coder` directly; `.cache` is writable here. The repo itself lives on GitHub.)
...
**Files:**
- Create: `/home/coder/.cache/coder-workspaces/flake.nix`
- Create: `/home/coder/.cache/coder-workspaces/image.nix`
- Create: `/home/coder/.cache/coder-workspaces/.gitignore`
- Create: `/home/coder/.cache/coder-workspaces/flake.lock` (generated, seeded)
...
- Produces: flake with `packages.x86_64-linux.coder-workspaces-nix` and `packages.x86_64-linux.default` (Task 2's workflow builds `.#coder-workspaces-nix`).

- [ ] **Step 1: Create the GitHub repo**

```bash
mkdir -p /home/coder/.cache/coder-workspaces
cd /home/coder/.cache/coder-workspaces
git init -b main
...
cp /home/coder/nixos-configurations/pkgs/coder-workspace/default.nix \
   /home/coder/.cache/coder-workspaces/image.nix
```
...
- [ ] **Step 6: Verify the build locally** *(sandbox: SKIP the heavy `nix build` — the dev sandbox has limited nix; the real build gate is the CI run in Task 2, plus a laptop-side `nix build .#coder-workspaces-nix` once the PR lands)*

```bash
cd /home/coder/.cache/coder-workspaces
nix build .#coder-workspaces-nix   # laptop, after PR; CI covers this too
```
...
**Files:**
- Create: `/home/coder/.cache/coder-workspaces/.github/workflows/build.yml`
- Create: `/home/coder/.cache/coder-workspaces/IMAGE_TAGS.md`
- Create: `/home/coder/.cache/coder-workspaces/README.md`

**Interfaces:**
- Consumes: Task 1's flake (`.#coder-workspaces-nix`), GHCR namespace `ghcr.io/javierarrieta/coder-workspaces-nix`.
    ...
    steps:
      ...
      - name: Load and tag
        run: |
          ...
          docker tag coder-workspace:pinned ghcr.io/javierarrieta/coder-workspaces-nix:${{ steps.tag.outputs.tag }}
          docker tag ghcr.io/javierarrieta/coder-workspaces-nix:${{ steps.tag.outputs.tag }} ghcr.io/javierarrieta/coder-workspaces-nix:latest

      - name: Push to GHCR
        run: |
          echo "${{ secrets.GITHUB_TOKEN }}" | docker login ghcr.io -u ${{ github.actor }} --password-stdin
          docker push ghcr.io/javierarrieta/coder-workspaces-nix:${{ steps.tag.outputs.tag }}
          docker push ghcr.io/javierarrieta/coder-workspaces-nix:latest
...
```bash
cd /home/coder/.cache/coder-workspaces
gh api user/packages/container/coder-workspaces-nix/versions --jq '.[0].name, .[0].metadata.container.tags[]'
```
...
pulls work — GitHub UI: Packages → `coder-workspaces-nix` → Package settings →
...
```markdown
- **Workspace image**: `ghcr.io/javierarrieta/coder-workspaces-nix:<tag>` (built in
...
1. Repoint `workspace_image` default in `coder-templates` `llm01-podman` template from `registry.l.arrieta.eu/coder-workspace:6fd2505` to `ghcr.io/javierarrieta/coder-workspaces-nix:<first-new-tag>`.
