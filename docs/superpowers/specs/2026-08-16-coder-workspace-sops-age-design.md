...
- `neovim` + `vim`: editor (none was present before); `vim` as a classic fallback. Per-project dotfiles (`init.lua`/`init.vim`) in the shared `/home/coder` are used if present. (nixpkgs attribute is `neovim`, not `nvim` — corrected from the initial commit.)
