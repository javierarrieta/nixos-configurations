...
comin only if the retry fails again. Note: after a rollback
`deployer.deployment.status` stays `"done"` — the approve script detects
rollback via `is_suspended`, not status.
